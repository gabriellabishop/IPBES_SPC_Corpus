...
params:
    ...
    search_terms: !expr >-
      list(
        spc_corpus =  yaml::read_yaml(file.path("input", "search_terms", "spc_corpus.v5.yaml")),
        chapter_1 = yaml::read_yaml(file.path("input", "search_terms", "chapter_1.v1.yaml")),
        chapter_3 = yaml::read_yaml(file.path("input", "search_terms", "chapter_3.v1.yaml")),
        chapter_4 = yaml::read_yaml(file.path("input", "search_terms", "chapter_4.v1.yaml")),
        chapter_6 = yaml::read_yaml(file.path("input", "search_terms", "chapter_6.v1.yaml"))
      )
...
if (!file.exists(fn)) {
  spcc <- paste0("(", stcompact(params$search_terms$spc_corpus$spc), ") AND (", stcompact(params$search_terms$spc_corpus$nature), ")")
  queries <- lapply(
    params$search_terms$chapter_1,
    function(s) {
      openalexPro::pro_query(
        title_and_abstract.search = paste0("(", spcc, ") AND (", stcompact(s), ")"),
        type = params$types_filter,
        from_publication_date = params$publication_date$from,
        to_publication_date = params$publication_date$to
      )
...
readRDS("spc_corpus/output/chapter_1/st_hits.rds") |> 
  dplyr::arrange(count)
```
...
params$search_terms$chapter_3 |> sapply(FUN = nchar) |> sapply(FUN = sum)

# readRDS("spc_corpus/output/chapter_3/st_hits.rds") |> 
#   dplyr::arrange(count)
```
...
readRDS("spc_corpus/output/chapter_4/st_hits.rds") |> 
  dplyr::arrange(count)
```
...
params$search_terms$chapter_6 |> sapply(FUN = nchar) |> sapply(FUN = sum)

# readRDS("spc_corpus/output/chapter_6/st_hits.rds") |> 
#   dplyr::arrange(count)
```
